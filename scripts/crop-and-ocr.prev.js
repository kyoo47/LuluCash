...
/** Rects for 1280x720 (your tuned positions) */
...
// Fallback percentage rects if size changes
const RECTS_PCT = {
  P2:[0.0836,0.397,0.0797,0.075],
...
// NEW: harden preprocessing (grayscale + threshold) and upscale 3x
async function preprocExtract(srcAbs, rect, outAbs){
  await sharp(srcAbs)
    .extract(rect)
    .resize({ width: rect.width*3, height: rect.height*3, kernel:"nearest" })
    .grayscale()
    .threshold(150)           // binary: makes black digits on white
    .png()
    .toFile(outAbs);
}
...
// NEW: tighter per-digit slices (padX/padY tuned)
async function readBySlices(baseRel, N, padX=0.12, padY=0.20){
  const abs = PUB(baseRel);
  const meta = await sharp(abs).metadata();
  const W = meta.width, H = meta.height;

  const segW = W / N;
  const px = Math.round(segW*padX);
  const py = Math.round(H*padY);
...
async function main(){
  ...
  const P2 = await readBySlices(outs.P2,2);
  const P3 = await readBySlices(outs.P3,3);
  const P4 = await readBySlices(outs.P4,4);
  const P5 = await readBySlices(outs.P5,5);
