...
// NEW PREPROCESS: upscale 4x, grayscale, normalize, gamma (no hard threshold)
async function preprocExtract(srcAbs, rect, outAbs){
  await sharp(srcAbs)
    .extract(rect)
    .resize({ width: rect.width*4, height: rect.height*4, kernel:"nearest" })
    .grayscale()
    .normalize()     // stretch contrast
    .gamma(0.8)      // darken strokes a bit
...
// NEW: slightly looser pads so we don’t clip a digit
async function readBySlices(baseRel, N, padX=0.10, padY=0.15){
